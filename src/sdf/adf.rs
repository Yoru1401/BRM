...
pub(crate) const CLEARANCE_MASK: u32 = 0xff_ffff;
...
pub(crate) fn brick_budget() -> u32 {
    command_line::value("--bricks")
        ...
        .clamp(1, MAX_SLOT_SIDE * MAX_SLOT_SIDE * MAX_SLOT_SIDE)
}
...
impl Adf {
    ...
    pub(crate) fn probe(&self, world_point: Vec3) -> (f32, bool) {
        ...
        if tag >= TAG_SOLID {
            ...
            let clear = (word & CLEARANCE_MASK).max(1) - 1;
            let reach = gap.min_element().max(0.0) + clear as f32 * size + self.range();
        ...
        let unit = self.fetch(word, inside);
...
pub(crate) fn triangles_of(mesh: &Mesh) -> Vec<[Vec3; 3]> {
    ...
    corners
        ...
        .filter_map(|corner| {
            ...
            (area.length_squared() > 1e-20).then_some(triangle)
        })
        .collect()
}

pub(crate) fn fit(triangles: &mut [[Vec3; 3]], widest: f32) {
    let (low, high) = span_of(triangles);
...
pub(crate) fn bake(triangles: &[[Vec3; 3]]) -> Adf {
    ...
    let mut voxel = command_line::value("--voxel").unwrap_or(widest / TARGET_VOXELS);
    let budget = brick_budget();
    loop {
        if let Some(baked) = try_bake(triangles, &boxes, low, high, voxel, budget) {
            return baked;
        }
        voxel *= COARSEN;
    }
...
fn try_bake(
    ...
    for (index, (corner_low, corner_high)) in boxes.iter().enumerate() {
        let first = ((*corner_low - reach - origin) / size)
            .floor()
            .clamp(Vec3::ZERO, last)
            .as_uvec3();
        let final_cell = ((*corner_high + reach - origin) / size)
            .floor()
            .clamp(Vec3::ZERO, last)
            .as_uvec3();
    ...
    for (index, list) in lists.iter().enumerate() {
        ...
        jobs.push((used, origin + cell.as_vec3() * size, list));
        used += 1;
    }
    ...
    let bias = 0.5 * 3f32.sqrt() * voxel;
    let tie = (voxel * 1e-3).powi(2);
    let pool = ComputeTaskPool::get_or_init(TaskPool::default);
    let batch = jobs.len().div_ceil(pool.thread_num().max(1) * 8).max(1);
    let blocks: Vec<Vec<(u32, Vec<u8>)>> = pool.scope(|scope| {
        for batch in jobs.chunks(batch) {
            scope.spawn(async move {
                batch
                    .iter()
                    .map(|(slot, corner, list)| {
                        (
                            *slot,
                            brick_voxels(*corner, voxel, range, bias, tie, triangles, boxes, list),
                        )
                    })
                    .collect()
            });
    ...
    for (slot, voxels) in blocks.into_iter().flatten() {
    ...
    measure_clearance(&mut page, bricks);
...
fn measure_clearance(page: &mut [u32], bricks: UVec3) {
    let width = bricks.x as usize;
    let plane = width * bricks.y as usize;
    let mut clearance = vec![u32::MAX; page.len()];
    let mut queue: Vec<usize> = Vec::new();

    for (index, word) in page.iter().enumerate() {
    ...
    while frontier < queue.len() {
        ...
        let next_clearance = clearance[index] + 1;
        for z in -1..=1 {
            for y in -1..=1 {
                for x in -1..=1 {
                    let next = cell + IVec3::new(x, y, z);
                    if next.cmplt(IVec3::ZERO).any() || next.cmpge(bricks.as_ivec3()).any() {
                        continue;
                    }
                    let at = next.x as usize + next.y as usize * width + next.z as usize * plane;
                    if clearance[at] != u32::MAX {
                        continue;
                    }
                    clearance[at] = next_clearance;
                    queue.push(at);
                }
            }
        }
    ...
    for (index, word) in page.iter_mut().enumerate() {
        let tag = *word >> TAG_SHIFT;
        if tag < TAG_SOLID {
            continue;
        }
        *word = (tag << TAG_SHIFT) | clearance[index].clamp(1, CLEARANCE_MASK);
    }
}
    ...
    for z in 0..SPAN {
        for y in 0..SPAN {
            for x in 0..SPAN {
                ...
                let signed = signed_distance(point, triangles, boxes, list, tie) - bias;
                let unit = ((signed + range) / (2.0 * range)).clamp(0.0, 1.0);
                voxels.push((unit * 255.0).round() as u8);
            }
        }
    }
    voxels
...
fn signed_distance(
    ...
    for index in list {
        let (corner_low, corner_high) = boxes[*index as usize];
    ...
    match (point - nearest).dot(pseudo) < 0.0 {
        true => -reach,
        false => reach,
    }
}
